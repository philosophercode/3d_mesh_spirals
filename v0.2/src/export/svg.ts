...
`;
  ...
  elements.forEach(element => {
    if (element.type === 'triangle') {
      ...
      svg += `    <polygon points="${tri.p1.x.toFixed(2)},${tri.p1.y.toFixed(2)} ${tri.p2.x.toFixed(2)},${tri.p2.y.toFixed(2)} ${tri.p3.x.toFixed(2)},${tri.p3.y.toFixed(2)}" fill="${backgroundColor}"/>\n`;
    } else {
      const seg = element.data;
      svg += `    <line x1="${seg.p1.x.toFixed(2)}" y1="${seg.p1.y.toFixed(2)}" x2="${seg.p2.x.toFixed(2)}" y2="${seg.p2.y.toFixed(2)}" stroke="${seg.color}" stroke-width="1" stroke-opacity="${seg.opacity}" stroke-linecap="round"/>\n`;
    }
  });
